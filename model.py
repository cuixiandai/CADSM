...
class MyModel(torch.nn.Module):
    def __init__(self,batch_size=32,bilinear=True):
        ...
        self.conv0 = torch.nn.Conv2d(in_channels=icc, out_channels=ic, kernel_size=3, stride=2, padding=1) 
        self.conv1 = torch.nn.Conv2d(in_channels=ic, out_channels=ic, kernel_size=3, stride=1, padding=1)
    ...
    def forward(self, x):
        ...
        position_ids = torch.arange(batch_size*ic*fs*fs, dtype=torch.long, device=x.device)  
        position_embeds = self.position_embedding(position_ids).reshape(batch_size, ic,fs,fs) 
        
        x1=x+position_embeds
        ...
        x=x.reshape(batch_size,-1,fs*fs)
